...
public class ListaConcatenataInt {
    ...
    //Lezione21
    /* verifica se ogni elemento è maggiore della media degli elementi precedenti */
    public boolean verifica() {
        if (this.lunghezza <= 1)
            return true;

        return verifica(this.testa.getInfo(), 1, this.testa.getSuccessore());
    }

    private static boolean verifica(int somma, int i, NodoInt it) {
        if (it == null)
            return true;

        if (it.getInfo() <= (somma/(float)i))
            return false;

        return verifica(somma + it.getInfo(), ++i, it.getSuccessore());
    }
}