...
public class ListaConcatenataInt {
    ...
    //esercizio Esercitazione10
    public boolean verificaPariDispari() {
        if (this.lunghezza < 3)
            return false;

        return verificaPariDispari(this.testa, -1, false);
    }

    private boolean verificaPariDispari(NodoInt it, int prec, boolean check) {
        if (it == null)
            return false;
        ...
        if (check) {
            ...
            if (it.getInfo() == prec)
                return verificaPariDispari(it.getSuccessore(), prec, check);

            return verificaPariDispari(it.getSuccessore(), it.getInfo(), false);
        }

        if (it.getInfo()%2 == 0) {
            if (it.getInfo() == prec)
                return verificaPariDispari(it.getSuccessore(), it.getInfo(), true);

            return verificaPariDispari(it.getSuccessore(), it.getInfo(), false);
        }

        return verificaPariDispari(it.getSuccessore(), -1, false);
    }
}