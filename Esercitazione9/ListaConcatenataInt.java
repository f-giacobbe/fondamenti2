...
public class ListaConcatenataInt {
    ...
    //esercizio Esercitazione9
    /* verifica se la somma dei valori in posizione i-1 e i nella lista è sempre la metà della somma
     * dei valori in posizione i e i+1.
     */
    public boolean verificaCoppie() {
        if (this.lunghezza < 3)
            return false;

        return verificaCoppie(this.testa, this.testa.getSuccessore(), this.testa.getSuccessore().getSuccessore());
    }

    private boolean verificaCoppie(NodoInt prec, NodoInt curr, NodoInt succ) {
        if (succ.getSuccessore() == null)
            return ((prec.getInfo() + curr.getInfo()) == (curr.getInfo() + succ.getInfo())/2f);

        if (!((prec.getInfo() + curr.getInfo()) == (curr.getInfo() + succ.getInfo())/2f))
            return false;

        return verificaCoppie(curr, succ, succ.getSuccessore());
    }
}